...
│   ├── 05-objectives.tex
│   ├── 06-reference_framework.tex
│   ├── 07-project_scope.tex
│   ├── 08-methodology.tex
...
```bash
latexmk -pdf -shell-escape -synctex=1 -interaction=nonstopmode -file-line-error -output-directory=build main.tex
```
...
```tex
\newcommand{\norm}[1]{\left \lVert #1 \right \rVert}        # Vector norms
\newcommand{\abs}[1]{\left \lvert #1 \right \rvert}         # Absolute values
